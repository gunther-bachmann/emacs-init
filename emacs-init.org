...
#+begin_src emacs-lisp
  ...
  (use-package ellama
    ...
    (defun ollama-server-on ()
      ...
      (message "wait for 5 seconds for ollama server to settle")
      (sit-for 5)
      (message "starting language model codellama")
      (start-process "codellama" "*codellama*" "ollama" "run" "codellama"))
...
- accenting windows
  ...
  #+begin_src emacs-lisp
    (defun gb/remove-window-accent (&optional window)
      (set-window-margins (or window (selected-window)) 2 0)
      (set-window-fringes (or window (selected-window)) 0 0 t nil))

    (defun gb/add-window-accent (&optional window)
      (set-window-margins (or window (selected-window)) 1 0)
      (set-window-fringes (or window (selected-window)) (frame-char-width) 0 t nil))

    (defun gb/selected-window-accent ()
      (interactive)
      (set-face-background 'fringe "#402040")
      (when (or exwm-class-name (and (windmove-find-other-window 'left)
                                  (with-selected-window (windmove-find-other-window 'left) exwm-class-name)))
        (set-face-background 'fringe "orange3"))
      (walk-windows
       (lambda (window)
         (if (eq window (selected-window))
             (gb/add-window-accent window)
           (gb/remove-window-accent window)))
       nil t))
