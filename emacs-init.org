      ...
      (defun gb/mode-line-padding ()
        (let ((r-length (if gb/disable-modeline-updates
                          ...
                          (+ (length (replace-regexp-in-string
                             ...
                             (if (and mlscroll-mode (gb/modeline-line-selected-window-active-p)) 5 0)))))
...
** own ui stuff
   #+BEGIN_SRC emacs-lisp
     (defun split-window-below-and-select-scratch (&optional arg)
       (interactive "P")
       (let* ((new-window (split-window-below arg)))
         (unless arg
     ...
     (defun split-window-above-and-select-scratch (&optional arg)
       ...
       (let* ((old-window (get-buffer-window))
              (new-window (split-window-below arg)))
         (unless arg
     ...
     (defun split-window-right-and-select-scratch (&optional arg)
       (interactive "P")
       (let* ((new-window (split-window-right arg)))
         (unless arg
           (set-window-buffer new-window "*scratch*"))
         (aw-switch-to-window new-window)
         new-window))
     ...
     (defun split-window-left-and-select-scratch (&optional arg)
       (interactive "P")
       (let* ((old-window (get-buffer-window))
              (new-window (split-window-right arg)))
         (unless arg
           (set-window-buffer old-window "*scratch*"))
         (aw-switch-to-window old-window)
         new-window))
      ...
      (if (not (executable-find "sqlite3"))
        ...
        (progn
          ...
          (use-package org-roam
            ...
            (org-notes (format "/home/%s/documents/roam" (user-login-name)))
      ...
      (when (and (package-installed-p 'magit)
        ...
        (defun gb/capture-new-script-file ()
          ...
          (if-let* ((repo-location (cdr (assoc "emacs-init" (magit-repos-alist))))
                    (s-filename (read-string "script-name (no extension, no spaces): ")))
              (progn
                (find-file (expand-file-name (format "%s.org" s-filename) "~/repo/emacs/emacs-init/utils/"))
                (insert (format (slurp (format "%s/utils/racket-cli-script.template.org" repo-location))
                                s-filename
                                s-filename
                                s-filename)))
