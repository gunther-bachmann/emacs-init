...
** shell
#+begin_src emacs-lisp
  (defun safe-shell-command-to-string (command)
    (let ((loc
           (if (buffer-file-name)
               (file-name-directory (buffer-file-name))
             default-directory)))
      (if (file-directory-p loc)
...
** minibuffer
   #+BEGIN_SRC emacs-lisp
     ...
     (custom-set-faces  '(minibuffer-prompt ((t (:foreground "#c062c0" :inverse-video t)))))
   ...
   #+BEGIN_SRC emacs-lisp
     (when (eq gb/completion-framework 'ivy)
       ...
       (use-package all-the-icons-ivy
         :ensure t)
       (defun gb/ivy-candidates-offset (cand) (make-string (if (or (and (boundp 'mini-frame-mode) mini-frame-mode) (and (boundp 'mini-popup-mode) mini-popup-mode) (and (boundp 'ivy-posframe-mode) ivy-posframe-mode)) 0 (max 0 (- (frame-width) (ivy-rich-minibuffer-width 0.77)))) ? ))
       (defun gb/ivy-candidates-offset-20 (cand) (make-string (if (or (and (boundp 'mini-frame-mode) mini-frame-mode) (and (boundp 'mini-popup-mode) mini-popup-mode)(and (boundp 'ivy-posframe-mode) ivy-posframe-mode)) 0 20) ? ))
                  ...
                  (defun gb/exwm-visualize-workspace-switch (newidx length display-width)
                    ...
                    (let* (
                      ...
                      (posframe-show my-posframe-buffer
                                     ...
                                     :right-fringe 0
                                     :timeout 2)
                      (when (get-buffer-window my-posframe-buffer)
                        (set-window-margins (get-buffer-window my-posframe-buffer) 2 1)
                        (set-window-fringes (get-buffer-window my-posframe-buffer) 0 0 t nil))
...
* TODO networking (integration, en/disable etc.) - make package of this
  ...
  #+begin_src emacs-lisp
    ...
    (defun gb/net-lan-switch-on ()
      "switch wired lan on"
      (interactive)
      (message (s-trim (safe-shell-command-to-string (format "sudo ifconfig --up %s" gb/net-lan-adapter)))))

    (defun gb/net-lan-switch-off ()
      "switch wired lan off"
      (interactive)
      (message (s-trim (safe-shell-command-to-string (format "sudo ifconfig --down %s" gb/net-lan-adapter)))))

    (defun gb/net-wlan-switch-on ()
      "switch wireless lan on, needs tlp to be installed"
      (interactive)
      (safe-shell-command-to-string "sudo systemctl start wpa_supplicant.service ") ;; just to make sure
      (message (s-trim (safe-shell-command-to-string "sudo wifi on"))))

    (defun gb/net-wlan-switch-off ()
      "switch wireless lan off, needs tlp to be installed"
      (interactive)
      (message (s-trim (safe-shell-command-to-string "sudo wifi off")))) ;; systemctl stop wpa_supplicant.service

    (defun gb/net-bluetooth-switch-on ()
      "switch bluetooth on, needs tlp to be installed"
      (interactive)
      (message (s-trim (safe-shell-command-to-string "sudo bluetooth on"))))

    (defun gb/net-bluetooth-switch-off ()
      "switch bluetooth off, needs tlp to be installed"
      (interactive)
      (message (s-trim (safe-shell-command-to-string "sudo bluetooth off"))))
...
#+end_src
* EVAL vterm - better terminal emulation
...
#+end_src
* EVAL goto-line-preview, goto-char-preview
#+begin_src emacs-lisp
...
- accenting windows
  ...
  #+begin_src emacs-lisp
    (defvar gb/currently-accented-window nil)
    (defvar gb/currently-accented-buffer nil)
    (defvar gb/accented-window-font-remap-cookie nil)
    (defvar gb/open-fringe-correction-window nil)

    (defun gb/remove-window-accent ()
      ;; (when gb/accented-window-font-remap-cookie 
      ;;   (face-remap-remove-relative gb/accented-window-font-remap-cookie)
      ;;   (setq gb/accented-window-font-remap-cookie nil))
      (when (and gb/currently-accented-window (window-live-p gb/currently-accented-window))        
        (set-window-margins gb/currently-accented-window 2 1)
        (set-window-fringes gb/currently-accented-window 0 0 t nil)
        (gb/correct-fringe-for-deaccented-window)
        (setq gb/open-fringe-correction-window gb/currently-accented-window))
      (message "removed accent %S %S" gb/currently-accented-window gb/currently-accented-buffer)
      (setq gb/currently-accented-window nil)
      gb/currently-accented-buffer nil)

    (defun gb/add-window-accent (&optional window)
      (let ((win (or window (selected-window))))
        (unless (equal win gb/currently-accented-window)
          (when gb/currently-accented-window
            (gb/remove-window-accent))  
          ;; (setq gb/accented-window-font-remap-cookie
          ;;       (face-remap-add-relative 'fringe `((:background ,(gb/background-for-window-accent win)))))
          (set-window-margins win 1 0)
          (set-window-fringes win (frame-char-width) (frame-char-width) t nil)
          (set-face-background 'fringe (gb/background-for-window-accent))
          (message "accented %S %S" win (window-buffer win))
          (setq gb/currently-accented-window win)
          (setq gb/currently-accented-buffer (window-buffer win))
          (run-at-time 1 nil #'gb/restore-default-fringe-color))))

    (defun gb/correct-fringe-for-deaccented-window ()
      (when gb/open-fringe-correction-window
        (when (window-live-p gb/open-fringe-correction-window)
          (set-window-margins gb/open-fringe-correction-window 1 0)
          (set-window-fringes gb/open-fringe-correction-window (frame-char-width) (frame-char-width) t nil))
        (setq gb/open-fringe-correction-window nil)))      

    (defun gb/restore-default-fringe-color ()
      (set-face-background 'fringe "#161414")
      (walk-window-tree #'gb/restore-window-fringe (selected-frame))
      ;; (gb/correct-fringe-for-deaccented-window)
      )

    (defun gb/background-for-window-accent (&optional window)
      (with-selected-window (or window (selected-window))
        (cond ((or ;; (one-window-p)
                  (minibufferp)
                  (equal " *my-posframe-buffer*" (buffer-name)))
               "#161414")
              ((or exwm-class-name
                  (and (windmove-find-other-window 'left)
                     (with-selected-window (windmove-find-other-window 'left) exwm-class-name))
                  (and (windmove-find-other-window 'up)
                     (with-selected-window (windmove-find-other-window 'up) exwm-class-name))
                  (and (windmove-find-other-window 'down)
                     (with-selected-window (windmove-find-other-window 'down) exwm-class-name)))
               "orange3")
              (t "#402040"))))

    (defun gb/remove-window-accent-on-non-selected (win)
    ...
    (defun gb/restore-window-fringe (win)
    ...
    (defun gb/selected-window-accent (&rest args)
      (interactive)
      (when (equal '(nil) (window-margins))
        (gb/set-default-margin))
      ;; (message "selected-window-accent-called")
      (if (or ;; (one-window-p)
             (minibufferp))
          (progn
            ;; (message "one-win or minibuff accent removed")
            (walk-window-tree #'gb/remove-window-accent-on-non-selected (selected-frame))
            ;; (gb/remove-window-accent)
            ;; (set-face-background 'fringe "#161414")
            (set-face-background 'fringe (gb/background-for-window-accent))
            )
        (progn  
          ...
          ;; ;; (message "regular accent %s" (buffer-name))
          ;; (when (or exwm-class-name
          ;;          (and (windmove-find-other-window 'left)
          ;;             (with-selected-window (windmove-find-other-window 'left) exwm-class-name))
          ;;          (and (windmove-find-other-window 'up)
          ;;             (with-selected-window (windmove-find-other-window 'up) exwm-class-name))
          ...
          (unless (or (one-window-p)
                     (equal " *my-posframe-buffer*" (buffer-name))
                     (minibufferp)
                     (and (equal (selected-window) gb/currently-accented-window)
                        (equal (current-buffer) gb/currently-accented-buffer)))
            (message "want to accented %S %S" (selected-window) (window-buffer (selected-window)))
            (walk-window-tree #'gb/remove-window-accent-on-non-selected (selected-frame))        
            (gb/remove-window-accent)        
            (gb/add-window-accent)))))

    ;; (add-hook 'buffer-list-update-hook #'gb/selected-window-accent)
    ;; (remove-hook 'buffer-list-update-hook #'gb/selected-window-accent)
    (add-hook 'window-configuration-change-hook #'gb/selected-window-accent)
    (add-hook 'window-state-change-hook #'gb/selected-window-accent)
  #+end_src
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (gb/do-after-display-system-init
     ...
     (cond
      ((eq gb/theme-mode 'light) nil) ;; TODO complete
      (t (progn
           (--each '((hl-line ((t (:extend t :background "#222239" :distant-foreground "#b0b0b0"))))
