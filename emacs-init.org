...
** dired-annotator
   #+begin_src emacs-lisp
     ...
     (use-package dired-annotator
       ...
       :config
       (require 'posframe)
       (defconst dired-annotator-note-popup-width 54 "width of the note that pops up (less since serif fonts are used)")
...
* EVAL mlscrollbar
  ...
  #+begin_src emacs-lisp
    (use-package mlscroll
      ...
      :config
      ;; (mlscroll-mode 1)
      ;; (defconst mlscroll-model-line-const (list "[" '(:eval (mlscroll-mode-line)) "]"))
  ...
  #+begin_src emacs-lisp
    (defun gb/url-reachable-p (url &optional timeout)
      ...
      (let ((local-timeout (or timeout 1)))
        (string-match-p "HTTP.*[0-9][0-9][0-9].*" (string-trim (shell-command-to-string (format "timeout %s curl -Is %s | head -n 1" (number-to-string local-timeout) url))))))
    ...
    (defun gb/slack-work-start (&optional channel-reg-ex msg)
      ...
      (let ((seconds-to-send-message 1))
        (unless gb/slack-modeline-status--timer
          (gb/slack-on)
          (setq seconds-to-send-message 10))
        (when msg
          (run-at-time
