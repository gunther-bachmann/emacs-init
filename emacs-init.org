      ...
      (when gb/dual-monitor-setup
        (progn
          ...
          (add-hook 'exwm-randr-screen-change-hook
                    (lambda ()
                      (shell-command "xrandr --output DP-2 --left-of DP-4 --auto")
                      (let* ((xrandrmode (s-trim (shell-command-to-string "xrandr -q | grep DP-4 -A 1 | tail -n 1 | awk '{ print $1; }'"))))
                        (shell-command (s-concat "xrandr --output DP-4 --mode " xrandrmode)))))
          (exwm-randr-enable)))
