     ...
     Argument BUFFER."
       ...
       (defun gb/helm-set-posframe-parameters ()
         ...
         (let ((xfac (cond ((eq 'helm-for-files (helm-this-command)) 0.8)
                           ...
                           ((eq 'gb/exwm-program-select (helm-this-command)) (if gb/presentation-mode 0.8 0.2))
                           ((eq 'helm-M-x (helm-this-command)) (if gb/presentation-mode 0.8 0.3))
                           (t (if gb/presentation-mode 0.8 0.5))))
            ...
            (let ((dual-monitor (string= "2" (s-trim (shell-command-to-string "xrandr -q | grep ' connected ' | wc -l"))))
              ...
              (if dual-monitor
                  (let* ((second-monitor (gb/get-second-monitor))
                         (sm-resolution (s-trim (shell-command-to-string (s-concat "xrandr | grep -e \"" second-monitor "\" -A 1 | tail -n 1 | tr -s \" \" | cut -d' ' -f2")))))
