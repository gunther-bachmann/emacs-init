...
* startup profile configuration
  #+begin_src emacs-lisp
    ...
    (setq gb/use-frame-completion nil) ;; 'ivy-posframe, 'mini-frame, nil
  #+end_src
* startup
#+begin_src emacs-lisp
   ...
   #+BEGIN_SRC emacs-lisp
     (when (eq gb/completion-framework 'ivy)
       ...
       (use-package all-the-icons-ivy
         :ensure t)
       (defun gb/ivy-candidates-offset (cand) (make-string (if (or (and (boundp 'mini-frame-mode) mini-frame-mode) (and (boundp 'ivy-posframe-mode) ivy-posframe-mode)) 0 (max 0 (- (frame-width) (ivy-rich-minibuffer-width 0.77)))) ? ))
       (defun gb/ivy-candidates-offset-20 (cand) (make-string (if (or (and (boundp 'mini-frame-mode) mini-frame-mode) (and (boundp 'ivy-posframe-mode) ivy-posframe-mode)) 0 20) ? ))
...
*** personal blogs (disabled)
 ...
 #+END_SRC
*** make org export use better highlighting with htmlize (disabled)
 #+BEGIN_SRC emacs-lisp :tangle no
    (use-package htmlize
      :ensure t)
 #+END_SRC
...
*** org appt (disabled) (notification of agenda items)
    #+BEGIN_SRC emacs-lisp :tangle no
      (use-package appt
        ;; :after ( org)
        ;; :defer 20
...
*** adaptive wrap (disabled)
    ...
    #+END_SRC
*** insertion of web images (disabled)
...
*** org table sticky headers (disabled)
    ...
    #+END_SRC
*** org pdftools (disabled) (must be loaded before org mode)
  ...
  - package
    #+begin_src emacs-lisp
      (use-package org-tree-slide
        ...
        (add-hook 'org-tree-slide-while-animation-sitting-hook #'gb/presentation-hide-tags)
        ...
        (defun gb/presentation-on ()
          ...
          (if gb/org-indent
              (org-indent-mode 1)
            (org-indent-mode -1))
          (when (boundp 'gb/presentation-bg-color)
        ...
        (progn
          ...
          (use-package org-roam
            ...
            (when (fboundp 'notdeft)
              (bind-key "C-c n f" 'org-roam-node-find 'notdeft-mode-map)
              (bind-key "C-c n n" 'org-roam-node-find 'notdeft-mode-map)
