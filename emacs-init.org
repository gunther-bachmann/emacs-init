    ...
    #+begin_src emacs-lisp
      ...
      (setq right-global-mode-string
            (append  (list  ;; eglot--mode-line-format
                      ...
                      mode-line-misc-info)
                     ;; '(:eval (when mode-line-process (format-mode-line (list mode-line-process))))
                     ))
...
*** org hide tags
    ...
    #+end_src
*** org tidy (hides properties drawers in overlay)
#+begin_src emacs-lisp
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package dired
       ...
       (setq dired-guess-shell-alist-user '(
                                            ("\\.webm\\'\\|\\.mkv\\'\\|\\.mpe?g\\'\\|\\.avi\\'\\|\\.mp4\\'\\|\\.flv\\'" "mpv")
                                            ("\\.d64\\'\\|\\.prg\\'" "x64sc")
                                            ("\\.sid\\'" "sidplayfp")
                                            ("\\.html?\\'" "firefox")
  ...
  #+BEGIN_SRC emacs-lisp
    (when (executable-find "direnv")
      (use-package envrc
        :ensure t
        :custom
        (envrc-on-lighter '((:propertize " Γ" face envrc-mode-line-on-face)))
        (envrc-error-lighter '((:propertize " Γ" face envrc-mode-line-error-face)))
        (envrc-none-lighter '((:propertize " Γ" face envrc-mode-line-none-face)))
        :config
        (envrc-global-mode 1)
...
** emms (emacs music system)
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package emms
       ...
       :custom (emms-mode-line-icon-color "goldenrod4")
       :config
       (require 'emms-setup)
       (emms-all)
    ...
    (when (executable-find "udisksctl")
      ...
      (defun gb/udisksctl--get-luks-mounts (device)
        ...
        (-map 
         #'cdr
         (--filter
          (< 0 (length (string-trim (shell-command-to-string (format "sudo cryptsetup status %s | grep -e 'device:.*%s'" (car it) device)))))
          (--map
           (cons (nth 0 (split-string it " ")) (nth 2 (split-string it " ")))
           (split-string (string-trim (shell-command-to-string "mount | grep -e '/dev/mapper.*/run/media'")) "\n")))))

      (defun gb/udisksctl--completing-read-mount (device)
        (when-let ((mounts (append (gb/udisksctl--get-mounts device) (gb/udisksctl--get-luks-mounts device))))
      ...
      (defun gb/udisksctl--unmount-mount (mount)
        ...
        (message (string-trim (shell-command-to-string (format "sudo umount %s" mount)))))
