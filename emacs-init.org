   ...
   #+BEGIN_SRC emacs-lisp
     (use-package tls
       ...
       (when gnutls-cli-installed
         ...
         (setq gnutls-log-level 0) ;; (0 = none, 1 = important, 2 = debug)
...
** font related stuff
   #+BEGIN_SRC emacs-lisp
     ...
     ;; (setq jit-lock-defer-contextually t ;; 'syntax-driven
     ;;       jit-lock-context-time 0.5
     ;;       jit-lock-defer-time 0
     ;;       jit-lock-stealth-load 50 ;; 200
     ;;       jit-lock-stealth-nice 0.5
     ;;       jit-lock-stealth-time nil ;; 0.2
     ;;   )
   ...
   - install ligatures via fira code symbol (had to install [[https://github.com/tonsky/FiraCode/files/412440/FiraCode-Regular-Symbol.zip][fira code symbol]] for the following to work)
     ...
     #+END_SRC
** sublimity (smooth scrolling, disabled)
   #+BEGIN_SRC emacs-lisp :tangle no
...
*** all the icons
    ...
    #+END_SRC
*** doom modeline
    - adjusted file doom-modeline-segments.el:276, changed "save" to "change_history" (icon)⃤∆
    - adjusted file doom-modeline-segments.el:856, changed "block" to "sim_card_alert"
    - adjusted file doom-modeline-core.el doom-modeline--font-height, change heigth to 120 (instead of calculating font height)
    - adjusted file doom-modeline-core.el doom-modeline-def-modeline, change (if (number-or-marker-p 120)...) (instead of calculation font
      stuff here)
    #+BEGIN_SRC emacs-lisp
        ...
        (defun doom-modeline--font-height ()
          ...
          (let ((height 100))
            (round (* 1.68 (cond ((integerp height) (/ height 10))
                                 ...
                                 (t (frame-char-height)))))))
        (defun doom-modeline--window-font-width () 9)
...
** COMMENT selfinsertion on readonly modes
   #+BEGIN_SRC emacs-lisp
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (use-package magit
        ...
        (progn
          ...
          (defun gb/translate-slack-name-to-id (member)
            ""
            (let ((slack-team-ids '(("Lukas" . "U5WGHU048")
                                 ("Roman" . "UHBBUGGJK")
                                 ("Aram" . "UH1GRNW65")
                                 ("Andreas" . "UHDEEH5D4")
                                 ("Damian" . "UHA9Y42KW")
                                 ("Philipp" . "U70T5PS6T")
                                 ("Olaf" . "U5DF9SC57"))))
              (cdr (--first (s-equals? member (car it)) slack-team-ids))))

          (defun gb/aws-pull-request (uni)
            "create pull request based on the current repo, put the url into the kill ring and (if NOT called with universial arg) browse the url"
            (interactive "P")
            (direnv-allow)
            (let* ((history '("Aram Roman Lukas" "Andreas Phillip Damian"))
                   (members (helm-read-string "slack-members:" (first history) 'history))
                   (memberlist (s-split " " members))
                   (memberids (--map (gb/translate-slack-name-to-id it) memberlist))
                   (member-parameters (--map (s-concat "-m \"" it "\"") memberids ))
                   (command (s-concat "aws-create-pullrequest -s \"#otto_eins_pr\" " (s-join " " member-parameters)))
                   (result (gb/shell-command-to-exit-code-and-string command))
                   (exit-code (car result))
                   (result-string (cdr result)))
              (message (s-concat "CALL: " command))
              (message (s-concat "AWS PullRequest Result String: " result-string))
              (message (s-concat "AWS PullRequest Result Code  : " (number-to-string exit-code)))
              (when (eq exit-code 0)
                (kill-new result-string)
                (unless uni
                  (browse-url result-string)))))
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package org
      ...
      (progn
        ...
        (custom-set-faces '(header-line ((t (:inherit org-table-face :foreground "#8abeb7"))))) ;; set (non scrolling) header to inhiert font from org mode tables (not mode line, which is to small on multi monitor w/ current config)
   ...
   #+begin_src emacs-lisp
     ...
     Argument BUFFER."
       ...
       (defun gb/helm-set-posframe-parameters ()
         ...
         (let ((xfac (cond ((eq 'helm-for-files (helm-this-command)) 0.8)
                           ((eq 'helm-mini (helm-this-command)) 0.8)
                           ((eq 'gb/exwm-program-select (helm-this-command)) (if (eq gb/dual-screen-mode 'presentation) 0.8 0.2))
                           ((eq 'helm-M-x (helm-this-command)) (if (eq gb/dual-screen-mode 'presentation) 0.8 0.3))
                           (t (if (eq gb/dual-screen-mode 'presentation) 0.8 0.5))))
               (yfac (cond ((eq 'helm-for-files (helm-this-command)) 0.8)
                           ((eq 'helm-mini (helm-this-command)) 0.8)
                           ((eq 'helm-M-x (helm-this-command)) 0.8)
                           (t (if (eq gb/dual-screen-mode 'presentation) 0.8 0.5)))))
...
** posframe
   ...
   - needed patching posframe.el:494
     #+begin_src emacs-lisp :tangle no
...
** comint settings
   #+begin_src emacs-lisp
     (custom-set-faces '(comint-highlight-prompt ((t (:foreground "peru")))))
...
* exwm (emacs window manager)
  ...
  - desktop functions
    #+begin_src emacs-lisp
      (defvar gb/volume-sink "0" "current audio output sink. list available ones with 'pactl list short sinks'.")
      (defun gb/audio-auto-select-sink ()
        "automatically select the most specific audio sink"
        (interactive)
        (let* ((usb-device (s-trim (shell-command-to-string "pactl list short sinks | grep -i usb- | cut -f1")))
               (built-in (s-trim (shell-command-to-string "pactl list short sinks | grep -i pci- | cut -f1"))))
          (setq gb/volume-sink (if (seq-empty-p usb-device)
                                   built-in
                                 usb-device))))
      ...
      (defun gb/audio-lower-volume ()
        ...
        (interactive)
        ;; (emms-volume-lower)
        (shell-command (format "pactl set-sink-volume %s -5%%" gb/volume-sink))
        (message (s-concat "volume down, now " (gb/get-volume-lr-escaped))))
      (defun gb/get-volume-lr-escaped ()
        ""
        (s-replace "%" "%%" (s-trim (shell-command-to-string (format "pactl list sinks | grep -A 9 \"Sink #%s\" | grep \"Volume:\" | awk '{ print $5 \" / \" $12 }'" gb/volume-sink)))))
                  ...
                  (defun gb/exwm-workspace-right ()
                    ...
                    (let* ((workspaceIdx (exwm-workspace--position exwm-workspace--current))
                      ...
                      (if (and (s-equals? (gb/get-primary-monitor) monitor)
                          ...
                          (progn (exwm-workspace-switch gb/exwm--previous-workspace t) t)
                  ...
                  (defun gb/exwm-workspace-down ()
                    ...
                    (let* ((workspaceIdx (exwm-workspace--position exwm-workspace--current))
                      ...
                      (if (or (and (s-equals? (gb/get-second-monitor) monitor)
                              ...
                              (and single-mon-p
                                   (< workspaceIdx lastIndex)))
                          (progn (exwm-workspace-switch (+ 1 workspaceIdx) t)
                  ...
                  (defun gb/exwm-workspace-up ()
                    ...
                    (let* ((workspaceIdx (exwm-workspace--position exwm-workspace--current))
                      ...
                      (if (or (and (s-equals? (gb/get-second-monitor) monitor)
                              ...
                              (and single-mon-p
                                   (> workspaceIdx 0)))
                          (progn (exwm-workspace-switch (- workspaceIdx 1) t)
      ...
      (when gb/use-exwm
        (use-package exwm-config
          ...
          (defun gb/refresh-exwm-monitor-setup ()
            ...
            (let ((dual-monitor (string= "2" (s-trim (shell-command-to-string "xrandr -q | grep ' connected ' | wc -l"))))
                  (primary-monitor (car (exwm-randr--get-monitors))))
              (message "exwm change hook")
              (if dual-monitor
                  (let* ((second-monitor (nth 2 (second (exwm-randr--get-monitors))))
                         (geometry (nth 3 (second (exwm-randr--get-monitors))))
                         (sm-resolution (format "%sx%s" (oref geometry :width)(oref geometry :height))))
                    ...
                    (cond
                     ...
                     (t
                      (progn
                        ...
                        (shell-command (s-concat "xrandr --output " second-monitor " --mode " sm-resolution))
          ...
          ;; set some specifics
          ;; kuchen
          ;; (define-key exwm-mode-map (kbd "k") #'(lambda () (interactive) (gb/filtered-insert-char ?k exwm-class-name)))
          ;; (define-key exwm-mode-map (kbd "K") #'(lambda () (interactive) (gb/filtered-insert-char ?K exwm-class-name)))
          ;; (define-key exwm-mode-map (kbd "u") #'(lambda () (interactive) (gb/filtered-insert-char ?u exwm-class-name)))
          ;; (define-key exwm-mode-map (kbd "c") #'(lambda () (interactive) (gb/filtered-insert-char ?c exwm-class-name)))
          ;; (define-key exwm-mode-map (kbd "h") #'(lambda () (interactive) (gb/filtered-insert-char ?h exwm-class-name)))
          ;; (define-key exwm-mode-map (kbd "e") #'(lambda () (interactive) (gb/filtered-insert-char ?e exwm-class-name)))
          ;; (define-key exwm-mode-map (kbd "n") #'(lambda () (interactive) (gb/filtered-insert-char ?n exwm-class-name)))

          ;; start programs
          (exwm-input-set-key (kbd "s-1") #'gb/exwm-program-select) ;; do not wrap, since it already resets exwm

          ;; itemis browsing
          (exwm-input-set-key (kbd "C-c a i b") #'gb/itemis-blue-ant-browse)
          (exwm-input-set-key (kbd "C-c a i c") #'gb/itemis-google-cal-browse)
          (exwm-input-set-key (kbd "C-c a i d") #'gb/itemis-google-drive-browse)
          (exwm-input-set-key (kbd "C-c a i l") #'gb/itemis-launchpad-browse)
          (defun gb/itemis-blue-ant-browse ()
            "browse to aws service console for code commit"
            (interactive)
            (browse-url "https://blueantasp32.proventis.net/itemis/psap"))
          (defun gb/itemis-google-cal-browse ()
            "browse to aws service console for code commit"
            (interactive)
            (browse-url "https://calendar.google.com"))
          (defun gb/itemis-google-drive-browse ()
            "browse to aws service console for code commit"
            (interactive)
            (browse-url "https://drive.google.com/drive/my-drive"))
          (defun gb/itemis-launchpad-browse ()
            "browse to aws service console for code commit"
            (interactive)
            (browse-url "https://intern.itemis.de/launchpad/"))

          (exwm-input-set-key (kbd "C-c a o j") #'gb/otto-jira-browse)
          (defun gb/otto-jira-browse ()
            "browse to aws service console for code commit"
            (interactive)
            (browse-url "https://jira.scm.otto.de"))
          ...
          (add-hook 'exwm-init-hook
                    (lambda ()
                      ...
                      ;; (exwm-mff-mode)
                      (setq global-mode-string '("" appt-mode-string emms-mode-line-string emms-playing-time-string " "
          ...
          (add-hook 'exwm-workspace-switch-hook #'gb/exwm-after-switch)
          ;; (defadvice exwm-layout--on-echo-area-change (after exwm-layout--on-echo-area-change--debug-jitter activate)
          ;;   (message "exwm-layout--on-echo-area-change executed"))
          )
        (defadvice switch-to-buffer (after switch-to-buffer-check-exwm-buffer activate)
          ...
          (when (and exwm-class-name
            ...
            (progn
              (message "native keyboard exwm settings applied (after switch to buffer).")
              ;; (exwm-input-release-keyboard (exwm--buffer->id (window-buffer)))
...
* networking en/disable
  #+begin_src emacs-lisp
    ...
    (defun gb/net-wlan-switch-on ()
      ...
      (interactive)
      (message (s-trim (shell-command-to-string "sudo systemctl start wpa_supplicant.service"))))

    (defun gb/net-wlan-switch-off ()
      "switch wireless lan off"
      (interactive)
      (message (s-trim (shell-command-to-string "sudo systemctl stop wpa_supplicant.service"))))
  #+end_src
...
* EVAL treemacs (start with C-x t t)
  ...
  #+begin_src emacs-lisp
    (use-package treemacs
      ...
      :config
      (custom-set-faces '(treemacs-root-face ((t (:inherit font-lock-variable-name-face :weight bold :height 1.2)))))
...
* EVAL filter annoying messages
  #+begin_src emacs-lisp
    ...
    (defvar message-filter-regexp-list '("^Starting new Ispell process \\[.+\\] \\.\\.\\.$"
                                         ...
                                         "^Invalid face .*$"
                                         "^no exwm keyboard modification applied.*$")
...
* EVAL pomodoro
  #+begin_src emacs-lisp
    (use-package pomodoro
      ...
      (setq pomodoro-sound-player "mpg123")
      (setq pomodoro-time-format "%.2m ")
      (pomodoro-add-to-mode-line))
  #+end_src
* EVAL org roam
  #+begin_src emacs-lisp :tangle no
    (if (file-exists-p "~/.emacs.d/additionals/org-roam/")
        (use-package org-roam
          :load-path "~/.emacs.d/additionals/org-roam"
          :after org
          :hook (org-mode . org-roam-mode)
          :custom
          (org-roam-directory "~/temp")
          :bind
          ("C-c n l" . org-roam)
  ...
  #+end_src
* EVAL emacs-mini-frame
  - now available as melpa package (but not patched yet)
  - mini-frame-do-commands was added by me
  #+begin_src emacs-lisp
    (use-package mini-frame
      :load-path "~/.emacs.d/additionals/mini-frame.el"
      :config
      (custom-set-variables
       '(mini-frame-show-parameters
         '((top . 10)
           ...
           (left . 0.5)
           (height . 2))))
      (setq mini-frame-do-commands '(exwm-workspace-switch))
