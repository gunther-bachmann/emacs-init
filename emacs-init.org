  ...
  (defun gb/play-startup-sound-when-ctrlkey-down ()
    ...
    (when (and (executable-find "getEventDeviceFor")
      ...
      (let* ((default-output "alsa_output.pci-0000_00_1f.3.analog-stereo")
             (tp-input (shell-command-to-string "getEventDeviceFor 'TouchPad'"))
     ...
     (custom-set-variables
      '(safe-local-variable-values
        '((org-tree-slide-content--pos)
          ...
          (gb/slides-started)
          (eval setq org-show-notification-handler
                #'(lambda
                    (msg)
                    (message msg)))
...
** font related stuff
   #+BEGIN_SRC emacs-lisp
     ...
     (setq jit-lock-defer-contextually nil ;; t ;; 'syntax-driven
           jit-lock-context-time 0.5
           jit-lock-defer-time 0.5
           jit-lock-stealth-load nil ;; 200
           jit-lock-stealth-nice 0.5
           jit-lock-stealth-time 0.2 ;; 0.2
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package sublimity
       ...
       (progn
         ...
         (add-to-list 'sublimity-handle-scroll-criteria '(not (memq major-mode (list 'ansi-term-mode 'term-mode 'shell-mode 'pdf-view-mode))))

         (setq auto-hscroll-mode t)
         (setq sublimity-auto-hscroll-mode t)
         (setq sublimity-scroll-drift-length 4)
         (setq sublimity-scroll-weight 4)
         (setq redisplay-dont-pause nil) ;; make sure not to pause until full redraw
...
*** doom modeline
    ...
    #+BEGIN_SRC emacs-lisp
      (use-package doom-modeline
        ...
        (custom-set-faces
         '(doom-modeline-bar ((t (:background "#600060"))))
         '(doom-modeline-bar-inactive ((t (:background "#500050"))))
         '(mode-line ((t (:background "#2C1529" :foreground "#808080" :weight normal))))
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package minimap
       ...
       (progn
         ...
         (custom-set-faces '(minimap-active-region-background ((t (:extend t :background "gray23")))))))
   #+END_SRC
     ...
     (when (boundp 'gb/private-google-calendar-url)
       ...
       (defun gb/import-diary-from-google-calendar-files ()
         ...
         (with-temp-buffer
           ...
           (icalendar-import-buffer gb/diary-calendar-file t t))
         (with-temp-buffer
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package org
      ...
      (progn
        ...
        :config
        (setq org-agenda-files '("~/slack.org"))
...
*** org appt (notification of agenda items)
    #+BEGIN_SRC emacs-lisp
      (use-package appt
        ...
        (custom-set-faces '(mode-line-emphasis ((t (:foreground "DarkGoldenrod3")))))
...
** slack
 #+BEGIN_SRC emacs-lisp
   ...
   (use-package slack
     ...
     (defun gb/slack-lui-push-button (&optional pos use-mouse-action)
       "make sure jitsy references are opened using chromium, all else is using firefox"
       (interactive
        (list (if (integerp last-command-event) (point) last-command-event)))
       (when (integerp pos)
         (let* ((button (button-at (or pos (point))))
                (luiaction (button-get button 'lui-button-function))
                (luiparameter (button-get button 'lui-button-arguments)))
           (if (eq luiaction 'browse-url)
               (if (string-match-p ".*\\(/meet\\.jit\\.si/\\|/teams\\.microsoft\\).*" (car luiparameter))
                   (progn (message "... with chromium")
                          (browse-url-chromium (car luiparameter)))
                 (message " ... with firefox")
                 (browse-url-firefox (car luiparameter)))
             (message "... neither of the two, use standard behaviour")
             (push-button pos use-mouse-action)))))
     (bind-key "<RET>" #'gb/slack-lui-push-button button-map)
     ...
     (defun gb/slack-programming-xtext ()
       "set slack status"
       (interactive)
       (slack-user-set-status-request (slack-team-select) ":xtext:" "programming")
       (gb/slack-user-set-active))

     (defun gb/slack-programming-racket ()
       "set slack status"
       (interactive)
       (slack-user-set-status-request (slack-team-select) ":racket-lang:" "enjoying lambdas")
       (gb/slack-user-set-active))

     (defun gb/slack-coffee-break ()
       "set slack status"
       (interactive)
       (slack-user-set-status-request (slack-team-select) ":coffee:" "break")
       (gb/slack-user-set-away))

     (defun gb/slack-food-break ()
       "set slack status"
       (interactive)
       (slack-user-set-status-request (slack-team-select) ":knife_fork_plate:" "mampf")
       (gb/slack-user-set-away))

     (defun gb/slack-feierabend ()
       "set slack status"
       (interactive)
       (slack-user-set-status-request (slack-team-select) ":hourglass:" "...")
       (gb/slack-user-set-away))
   ...
   (use-package alert
    ...
    (alert-define-style
      'gb/alert-style :title
      "Make Org headings for slack messages I receive - Style"
      :notifier
      (lambda (info)
        (write-region
         (s-concat
     ...
     (setq alert-user-configuration
           '(((:category . "slack")) gb/alert-style nil))
     ...
     ;; ;; alert me on some channels
     ;; (add-to-list
     ;;  'alert-user-configuration
     ;;  '(((:title . "\\(nord\\|Slackbot\\|generali\\)")
     ;;     (:category . "slack"))
     ;;    message nil))
     )
 #+END_SRC
   ...
   #+begin_src emacs-lisp
     ...
     Argument BUFFER."
       ...
       (defun gb/helm-set-posframe-parameters ()
         ...
         (let ((xfac (cond ((eq 'helm-for-files (helm-this-command)) 0.77)
      ...
      (when gb/use-exwm
        (use-package exwm-mff
          ...
          (defun gb/mouse-pointer-to-cursor ()
            (interactive)
            (if (and (bound exwm-class-name) exwm-class-name)
      ...
      (when gb/use-exwm
        (use-package exwm-config
          ...
          (defun gb/pass-open-on-workspace-0 ()
            ...
            (unless (s-equals? (buffer-name) gb/password-store)
              (split-window-right-and-select-scratch)
  ...
  - package
    #+begin_src emacs-lisp
      (use-package org-tree-slide
        ...
        :commands (gb/presentation-start gb/presentation-end)
        :config
        (setq org-tree-slide-header nil)
        (setq org-tree-slide-heading-emphasis t)

        (defun gb/presentation-start ()
        ...
        (use-package org-roam
          ...
          :bind (:map org-roam-mode-map
                      ...
                      (("C-c n i" . org-roam-insert)
                       ("C-c n n" . notdeft-org-link-new-file)
...
* EVAL agda
  ...
  #+begin_src emacs-lisp
    ...
    (custom-set-faces '(agda2-highlight-datatype-face ((t (:foreground "light blue"))))
...
* EVAL beginend - jump to the right beginning / end, depending on the mode
  #+begin_src emacs-lisp
    (use-package beginend
      :ensure t
      :config (beginend-global-mode))
  #+end_src
* EVAL scratch - open scratch buffer in the same mode as the current buffer
  #+begin_src emacs-lisp
    (use-package scratch
      :ensure t
      :bind ("s-x" . scratch)
...
* EVAL eshell-syntax-highlighting
  ...
  #+begin_src emacs-lisp
    (use-package eshell-syntax-highlighting
      :after esh-mode
      :load-path "~/.emacs.d/additionals/eshell-syntax-highlighting/"
...
** EVAL magit-delta (disabled - speed degradation on large change sets)
   #+begin_src emacs-lisp :tangle no
     (use-package magit-delta
       ...
       (magit-delta-mode))
   #+end_src
** EVAL (disabled: does not compile) vterm - better terminal emulation
