     ...
     (when (eq gb/completion-framework 'ivy)
       ...
       (use-package ivy
         ...
         :custom ( ;; ivy-sort-functions-alist :: sorting candidates list is set in ivy-prescient
                  ...
                  ;; (ivy-ignore-buffers ("\\` " "\\`\\*tramp/" "\\`\\*Bookmark/"))
                  )
         :config
    ...
    - margins
      #+begin_src emacs-lisp
        ...
        (defun gb/set-default-margin-in-all-frames ()
          (--each (frame-list) (with-selected-frame it (--each (window-list) (with-selected-window  it (gb/set-default-margin))))))
      #+end_src
  - exwm configuration
    #+BEGIN_SRC emacs-lisp
  ...
  #+begin_src emacs-lisp
    ...
    (defun gb/add-window-accent (&optional window)
      ...
      (let ((win (or window (selected-window))))
        ...
        (set-window-fringes win (frame-char-width) 0 t nil)
        (setq gb/currently-accented-window win)
        (setq gb/currently-accented-buffer (current-buffer))))
      ...
      (if (or (one-window-p)
        ...
        (progn  
          ...
          (unless (or (one-window-p)
                     (minibufferp) (and (equal (selected-window) gb/currently-accented-window)
    ...
    ;; (add-hook 'window-state-change-hook  #'gb/selected-window-accent)
    ;;(add-hook 'after-focus-change-function #'gb/selected-window-accent)
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (gb/do-after-display-system-init
     ...
     (cond
      ((eq gb/theme-mode 'light) nil) ;; TODO complete
      (t (progn
           (--each '((hl-line ((t (:background "#202035"))))
