             ...
             (let ((message-log-max nil)  ; do not insert to *Messages* buffer
               ...
               (dolist (lst symon--display-fns)
                 (if (= page symon--active-page)
                     (message (concat (make-string  (- (/ (frame-pixel-width) 10) 80) ? ) "%s") (apply 'concat (mapcar 'funcall lst)))
                   (mapc 'funcall lst))
                 (setq page (1+ page))))
       ...
       (req-package exwm-config
         ...
         (defun gb/start-if-not-running (filename)
           ...
           (let* ((program (file-name-base filename)))
             (if (and (file-executable-p filename)
                      (string-empty-p (shell-command-to-string (concat "ps -A | grep " program))))
                 (async-shell-command filename nil nil)
               (message (concat "WARNING: program could not be run: " filename)))))
