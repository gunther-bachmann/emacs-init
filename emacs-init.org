...
* TODO backup duplicity support - make package
  #+begin_src emacs-lisp
    ...
    (defun gb/backup-timer-status ()
      "stop the timer that starts backups"
      (interactive)
      (gb/systemd-service-command gb/backup-timer-name "status" :user-service? t))
...
* TODO dired-annotator - make package of this
  #+begin_src emacs-lisp
    ...
    (defvar dired-annotator-note-popup-width 56 "width of the note that pops up")
    (defvar dired-annotator-note-popup-height 40 "height of the note that pops up")
    ...
    (defun dired-annotator-show-note ()
      ...
      (when-let* ((absolute-file-name (dired-x-guess-file-name-at-point))
        ...
        (let ((pf (posframe-show (find-file-noselect annotation-abs-file-name) :position (point) :lines-truncate t
  ...
  #+end_src
#+BEGIN_SRC emacs-lisp
(report-elapsed "check-point - dired annotator")
#+END_SRC
...
#+END_SRC
* TODO org babel tangle utilities
  #+begin_src emacs-lisp
    (require 'dired-x)
    (require 'dash)
    ...
    (defun gb/org-babel-tangle-folder (folder)
      ...
      (when (file-directory-p folder)
        (->> (directory-files folder)
          (--filter (string-match-p ".*\.org$" it) )
          (--map (org-babel-tangle-file (format "%s/%s" folder it)))
          (-flatten))))

    (defun gb/diredp--babel-tangle-folder ()
      "interactively guess the folder and tangle all org files within"
      (interactive)
      (if-let* ((folder (dired-x-guess-file-name-at-point))
