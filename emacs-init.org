       ...
       (use-package ivy
         ...
         (defun gb/ivy--program-select-and-run ()
           ...
           (unless gb/external-commands
             (setq gb/external-commands (gb/collect-external-commands nil)))
     ...
     (when (eq gb/completion-framework 'ivy)
       ...
       (use-package counsel
         ...
         :bind (( "M-x"     . counsel-M-x)
                ...
                :map comint-mode-map
                ( "<M-up>"  . gb/counsel-shell-history-with-current-line ))
         :config
         (progn
           (require 'comint)
           (defun gb/counsel-shell-history-with-current-line ()
             (interactive)
             (let ((ivy-completion-beg (comint-line-beginning-position))
                   (ivy-completion-end (point))
                   (initial-input (buffer-substring (comint-line-beginning-position) (point))))
               (ivy-read "History: " (ivy-history-contents comint-input-ring)
           ...
           (setcdr (assoc 'counsel-M-x ivy-initial-inputs-alist) "") ;; remove ^ from input
