       ...
       (let* ((current-dir (dired-current-directory))
         ...
         (term-send-string
          ...
          (if (file-remote-p wanted-dir)
              (let ((v (tramp-dissect-file-name wanted-dir t)))
                (format "ssh %s@%s\n"
                        (aref v 1) (aref v 2)))
            (format "cd '%s'\n" wanted-dir)))))
...
** switch buffers
   ...
   (use-package nswbuff
     ...
     :bind (("s-2" . nswbuff-switch-to-next-buffer)
            ("s-@" . nswbuff-switch-to-previous-buffer)))
   #+end_src
** terraform
   #+begin_src emacs-lisp
...
#+END_SRC
  ...
  (when gb/use-exwm
      ...
      :init (progn
              ...
              (defun gb/exwm-visualize-workspace-switch (newidx length display-width)
                ...
                (let* (
                  ...
                  (posframe-show " *inpos-posframe-buffer"
                                 :string (s-concat "workspace [" (number-to-string newidx) "]")
                                 :position (point)
                                 :timeout 1
              ...
              (defun gb/exwm-workspace-left ()
                ...
                (let* ((workspaceIdx (exwm-workspace--position exwm-workspace--current))
                  ...
                  (unless (or (s-equals? (gb/get-primary-monitor) monitor)
              ...
              (defun gb/exwm-workspace-right ()
                ...
                (let* ((workspaceIdx (exwm-workspace--position exwm-workspace--current))
                       (monitor (nth (+ 1 (* 2 workspaceIdx)) exwm-randr-workspace-monitor-plist)))
                  (when (and (s-equals? (gb/get-primary-monitor) monitor)
                             (not (string-empty-p (gb/get-second-monitor))))
                    (exwm-workspace-switch gb/exwm--previous-workspace)
                    t)))
              (defun gb/exwm-workspace-down ()
                ...
                (let* ((workspaceIdx (exwm-workspace--position exwm-workspace--current))
                  ...
                  (when (or (and (s-equals? (gb/get-second-monitor) monitor)
    ...
    (use-package exwm-config
      ...
      (exwm-input-set-key (kbd "s-2") #'nswbuff-switch-to-next-buffer)
      (exwm-input-set-key (kbd "s-@") #'nswbuff-switch-to-previous-buffer)
