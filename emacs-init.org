...
** font related stuff
#+BEGIN_SRC emacs-lisp
  ;; TODO: bring this into the gb/setup-main-fonts function !
  (if (gb/on-mac-p)
  ...
  (require 'ht)
  (defun gb/get-font-heights (display-width) 
  ...
  (defun gb/setup-main-fonts ()
    ...
    (let* ((attr-map (ht-merge (gb/get-font-heights (x-display-pixel-width))
...
** spinner (busy indicator for modeline)
   ...
   (req-package spinner)
   #+END_SRC
** rich minority mode (hide minor modes from mode line)
   #+BEGIN_SRC emacs-lisp
...
** form feed (^L) as h-bar
   #+BEGIN_SRC emacs-lisp
     (req-package form-feed
       :config
       (progn (add-hook 'emacs-lisp-mode-hook 'form-feed-mode)
