    ...
    #+BEGIN_SRC emacs-lisp
      (use-package ediff
        ...
        (custom-set-faces '(diff-removed ((t (:background "red" :foreground "#663333"))))
                          ...
                          '(magit-diff-added ((t (:background "#223822" :foreground "#ddffdd"))))
       ...
       #+begin_src emacs-lisp
         ...
         (defun gb/audio-lower-volume ()
           ...
           (let ((output-selected (gb/audio-output-selected)))
             ...
             (message (format "volume down, now %s     (%s)" (gb/get-volume-lr-escaped output-selected) output-selected))))

         (defun gb/get-volume-lr-escaped (element)
         ...
         (defun gb/audio-raise-volume ()
           ...
           (let ((output-selected (gb/audio-output-selected)))
             (shell-command (format "pactl set-sink-volume %s +5%%" output-selected))
             (message (format "volume up, now %s     (%s)" (gb/get-volume-lr-escaped output-selected) output-selected))))
         ...
         (defun gb/audio-input-select ()
           "select audio input"
           (interactive)
           (let ((source-name (helm-comp-read "audio-input:" (gb/audio-inputs-available) :must-match t)))
...
* EVAL magit-delta
  #+begin_src emacs-lisp
    (use-package magit-delta
      ...
      (magit-delta-mode))
  #+end_src
* EVAL ytel (youtube interface via invidous) [disabled, needs youtube-dl as extractor, too]
  #+begin_src emacs-lisp :tangle no
    (use-package ytel
      :config
      (defun ytel-watch ()
