  ...
  (when (and (executable-find "getEventDeviceFor")
    ...
    (let* ((tp-input (shell-command-to-string "getEventDeviceFor 'TouchPad'"))
      ...
      (when (and (or (equal "ok" kb-play-ok)
        ...
        (shell-command-to-string (format "pactl set-default-sink %s" "alsa_output.pci-0000_00_1f.3.analog-stereo"))
        (shell-command-to-string "pactl set-sink-volume 0 30%")
...
** font related stuff
   ...
   - speedup frame creation by caching font list during setup
     #+begin_src emacs-lisp
       ...
       ;; execute the following to get original behaviour
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (use-package beacon
       ...
       (defun gb/blink-if-not-shell (&rest args)
         ...
         (unless (memq major-mode (list 'ansi-term-mode 'term-mode 'shell-mode 'comint-mode))
           (beacon-blink)))
