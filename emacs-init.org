    ...
    (when (is-_nix-p)
      (progn
        ...
        (when (and chromium-installed firefox-installed)
          (let ((chrome-regex ".*\\(/meet\\.jit\\.si/\\|/teams\\.microsoft\\|adventure-hh\\|/meet\\.google\\.com/\\).*"))
   ...
   #+begin_src emacs-lisp
     (when (boundp 'gb/private-google-calendar-url)
       ...
       (defun gb/sync-update-agenda-with-google-calendar ()
         ...
         (shell-command-to-string (format "wget -q -O %s-temp %s" gb/itemis-nord-calendar-file gb/itemis-nord-google-calendar-url))
       ...
       (defun gb/import-diary-from-google-calendar-files ()
         ...
         (--each `(,gb/diary-calendar-file ,gb/itemis-calendar-file ,gb/itemis-nord-calendar-file ,gb/private-calendar-file)
           (progn
             ...
             (when (file-exists-p it) ;; delete diary-calendar-file also, since it needs to be rewritten!
               (delete-file it))))
         (--each `(,gb/private-calendar-file ,gb/itemis-calendar-file ,gb/itemis-nord-calendar-file)
         ...
         (with-temp-buffer
           (insert-file gb/itemis-nord-calendar-file)
           (icalendar-import-buffer (format "%s-temp" gb/diary-calendar-file) t t))
         (with-temp-buffer
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package org
      ...
      (progn
        ...
        :config
        (setq org-startup-folded 'fold) ;; "ensure opened org files are opened in overview (folded)"
...
*** org ref
    ...
    #+BEGIN_SRC emacs-lisp
      (use-package org-ref
        ...
        :custom (org-latex-pdf-process (list "latexmk -shell-escape -bibtex -f -pdf %f"))
    ...
    #+BEGIN_SRC emacs-lisp
      (when (and gb/profile--exwm gb/use-exwm)
        (use-package exwm-config
          ...
          (exwm-input-set-key (kbd "C-M-s-<left>") #'windmove-swap-states-left)
          (exwm-input-set-key (kbd "C-M-s-<right>") #'windmove-swap-states-right)
          (exwm-input-set-key (kbd "C-M-s-<up>") #'windmove-swap-states-up)
          (exwm-input-set-key (kbd "C-M-s-<down>") #'windmove-swap-states-down)
          ...
          (exwm-input-set-key (kbd "H-s-.") #'gb/slack-catchup-all-less-important-rooms)
...
* TODO backup duplicity support - make package
  #+begin_src emacs-lisp
    (defvar gb/backup-service-name "backup-drive.service" "the systemd service name that is used to start backups")
    ...
    (cl-defun gb/systemd-service-command (service-name command &key user-service?)
      ...
      (let ((user-param (if user-service? "--user" "")))
        (shell-command-to-string (format "systemctl %s %s %s" user-param command service-name))))

    (defun gb/backup-stop-timer-service-off ()
      "stop the timer that starts backups"
      (interactive)
      (gb/systemd-service-command gb/backup-service-name "stop" :user-service? t))

    (defun gb/backup-timer-service-on ()
      "start the timer that starts backups"
      (interactive)
      (gb/systemd-service-command gb/backup-service-name "start" :user-service? t))
  ...
  #+end_src
* TODO dired-annotator - make package of this
  #+begin_src emacs-lisp
    (use-package uuid
      :ensure t)

    (require 'simple)
    (require 'all-the-icons) ;; TODO: make optional (use unicode char)
    (require 'uuid)
    ...
    ;; -------------------------------------------------------------------------------- package: make this customizable
    (defvar dired-annotator-annotations-folder "/home/pe/documents/annotations" "location for annotation files")
    (defvar dired-annotator-note-icon (all-the-icons-faicon "sticky-note") "icon to use for note indication in dired buffer")
    ...
    (defvar-local dired-annotator--icons-shown nil "are icons currently shown in the dired buffer")

    ;; make sure folder exists
    (unless (file-directory-p dired-annotator-annotations-folder)
      (make-directory dired-annotator-annotations-folder t))
    ...
    ; keep a list with ('md5hash 'last-file-location 'last-file-name 'last-file-size 'org-file-id 'pinning-mode)
    ; md5hash            : md5hash of the first x bytes of the file
    ...
    (defun dired-annotator--collect-file-information (file-name)
      ...
      (let* ((abs-file (file-truename file-name))
        ...
        (list (dired-annotator--md5 file-name)
    ...
    (defun dired-annotator--fi-md5 (file-information)
      "get md5 hash from file-information"
    ...
    (defun dired-annotator--match-p (file-information-a file-information-b pinning-mode)
      ...
      (cond ((eq pinning-mode 'immutable-file)
             (let ((a-size (dired-annotator--fi-size file-information-a))
                   ...
                   (a-md5 (dired-annotator--fi-md5 file-information-a))
                   (b-md5 (dired-annotator--fi-md5 file-information-b)))
               (and (= a-size b-size) (string-equal a-md5 b-md5))))
    ...
    (defun dired-annotator--load-annotation-info-from-folder ()
      ...
      (setq dired-annotator--md5-2-annotation (make-hash-table :test 'equal))
    ...
    (defun dired-annotator--unhash-file (md5-key absolute-file-name)
      "remove given keys from hash

    MD5-KEY is the md5sum of the annotated file
    ABSOLUTE-FILE-NAME is the absolute file name of the annotated file"
      (puthash md5-key nil dired-annotator--md5-2-annotation)
      ...
      (let ((pinning (dired-annotator--fi-pinning file-information)))
        (cond ((eq pinning 'immutable-location)
              ...
              (t ;; (eq pinning 'immutable-file)
               (puthash (dired-annotator--fi-md5 file-information) annotation-file-name dired-annotator--md5-2-annotation)))))
    ...
    (defun dired-annotator--get-annotation-for (absolute-file-name)
      ...
      (let ((file-information (dired-annotator--collect-file-information absolute-file-name)))
        (or (gethash (dired-annotator--fi-md5 file-information) dired-annotator--md5-2-annotation)
    ...
    (defun dired-annotator-show-icons ()
      ...
      (interactive)
      (setq dired-annotator--icons-shown t)
               ...
               (save-excursion
                 ...
                 (loop-until (eobp)
                   (when (dired-move-to-filename nil)
                     (incf file-count)
                     (when-let* ((file (dired-x-guess-file-name-at-point))
                       ...
                       (beginning-of-line)))
                   (forward-line 1))))))
        (message (format "found %d notes looking at %d files in %f seconds" annotation-count file-count time))))

    (defun dired-annotator-hide-icons ()
      "Remove all `dired-annotator' overlays."
      (interactive)
      (setq dired-annotator--icons-visible nil)
      (save-restriction
        (widen)
        (mapc #'delete-overlay
              (dired-annotator--overlays-in (point-min) (point-max)))))

    (defun dired-annotator-edit-note ()
      "create a new annotation and open it, or open an existing"
      (interactive)
      (let* ((absolute-file-name (dired-x-guess-file-name-at-point))
             (annotation (or (dired-annotator--get-annotation-for absolute-file-name)
                            (when-let* ((new-annotation
                              ...
                              (when dired-annotator--icons-shown (dired-annotator--add-note-icon-to-line))
                              new-annotation)))
    ...
    (defun dired-annotator-delete-note ()
      ...
      (interactive)
      (when-let* ((annotated-file-name (dired-x-guess-file-name-at-point))
        ...
        (when (yes-or-no-p "Really delete this annotation? ")
          ...
          (dired-annotator--unhash-file (dired-annotator--md5 annotated-file-name) annotated-file-name)
          (delete-file annotation-file-name))))

    (defun dired-annotator-show-note ()
      "display the annotation in a posframe (if existent)"
      (interactive)
      (when-let* ((absolute-file-name (dired-x-guess-file-name-at-point))
        ...
        (let ((pf (posframe-show (find-file-noselect annotation-abs-file-name) :position (point) :lines-truncate t :width 40 :height 40
    ...
    ;; -------------------------------------------------------------------------------- package: put this in the 'bind-key' section
    (bind-key "C-c a s" #'dired-annotator-show-icons dired-mode-map)
    (bind-key "C-c a h" #'dired-annotator-hide-icons dired-mode-map)
    (bind-key "C-c a k" #'dired-annotator-delete-note dired-mode-map)
    (bind-key "C-c a o" #'dired-annotator-edit-note dired-mode-map)
    (bind-key "C-c a a" #'dired-annotator-show-note dired-mode-map)
    (bind-key "\"" #'dired-annotator-show-note dired-mode-map)
...
* TODO udisksctl - make package of this
  #+begin_src emacs-lisp
    ...
    (defun gb/udisksctl--get-mounts (device)
      (--filter
       (not (and (string-match-p (format ".*%s$" gb/backup-mount) (nth 2 (split-string it " "))) (gb/backup-is-running)))
