   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (use-package beacon
       ...
       (defun gb/blink-if-not-shell (&rest args)
         ...
         (unless (memq major-mode (list 'ansi-term-mode 'term-mode 'shell-mode 'comint-mode 'slack-mode 'slack-message-buffer-mode))
           (beacon-blink)))
...
** company
   #+BEGIN_SRC emacs-lisp
     ...
     ;; company box is patched!
     ;; see function company-box-doc--show
     ;;
     ;; ;; (unless (frame-live-p (frame-parameter nil 'company-box-doc-frame)))
     ;; ;; don't reuse the given frame, always create a new one
   ...
   #+begin_src emacs-lisp
     (when (boundp 'gb/private-google-calendar-url)
       ...
       (defun gb/update-agenda-with-google-calendar ()
         ...
         (async-start (lambda ()
                        (add-to-list 'load-path "~/.emacs.d/accounts/")
                        (require 'private)
                        (shell-command-to-string (format "wget -q -O ~/cal.ics %s" gb/itemis-google-calendar-url))
                        (shell-command-to-string (format "wget -q -O ~/pcal.ics %s" gb/private-google-calendar-url))
                        (when (file-exists-p gb/diary-calendar-file)
                          (delete-file gb/diary-calendar-file))
                        (icalendar-import-file gb/itemis-calendar-file gb/diary-calendar-file)
                        (icalendar-import-file gb/private-calendar-file gb/diary-calendar-file))
                      (lambda (result)
       ...
       (defun gb/start-update-agenda-timer ()
         ...
         (interactive)
         (setq gb/update-agenda-timer (run-at-time nil (* 5 60) #'gb/update-agenda-with-google-calendar)))
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package org
      ...
      (progn
        ...
        (setq gb/use-optimized-folded-src-fontificat t) ;; fontify src blocks only if they ar not folded!
...
** slack
 #+BEGIN_SRC emacs-lisp
   ...
   (use-package slack
     ...
     (bind-key "@" #'slack-message-embed-mention slack-mode-map)
     ...
     (setq tracking-ignored-buffers '(".*itemis : general.*" ))
...
* EVAL keypression
  - patched function keypression--push-string, self-insert evaluates to: (note check on org-self-insert-command)
    ...
    #+end_src
  - patched function keypression--key-to-string: (note check on org-self-insert-command)
    #+begin_src emacs-lisp :tangle no
      (or (eq this-command 'self-insert-command)
          (eq this-command 'org-self-insert-command))
    #+end_src
