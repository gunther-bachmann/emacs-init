...
** todo check existence of gnu ls (for dired quick sort)
...
** correct shell path behaviour
  ...
  (when (gb/on-mac-p)
    ...
    (exec-path-from-shell-copy-envs
     '("PATH")))
#+END_SRC
** check installation fo macos
   #+BEGIN_SRC emacs-lisp
   (when (gb/on-mac-p)
      (progn

        (check-_nix-program-status gnu-ls-installed "gls" "gls" "gnu ls not installed, dired quick sort will not work.")
  ...
  #+BEGIN_SRC emacs-lisp
    (if (is-_nix-p)
        (progn
          ...
          (unless (gb/on-mac-p) ;; otherwise gnu-ls-installed would be set by mac os initialization code
              (setq gnu-ls-installed (is-linux-p)))
          ))
  #+END_SRC
    ...
    #+BEGIN_SRC emacs-lisp
      (req-package spaceline
        ...
        (defun gb/vc-status ()
          (if vc-mode
              vc-mode
              (concat "\ue0a0 " (s-trim (shell-command-to-string "git rev-parse --abbrev-ref HEAD")))))
        (defun gb/git-state ()
           (let* ((git-output (shell-command-to-string "git status")))
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (org-babel-do-load-languages
       ...
       '((java . t)
         ...
         (emacs-lisp . t)
         (gnuplot . t)
         (groovy . (version<= "9.0.5" org-version))
         ;; (max . t) ;; maxima
         (plantuml . t)
         ;; (prolog . t)
         (python . t)
         (scala . (version<= "9.0.5" org-version))
         (scheme . t)
         (sed . (version<= "9.0.5" org-version))
         (shell . t)
         ;; (sml . t) ;; standard ml
         (sql . (version<= "9.0.5" org-version))))
...
** configuration
   #+BEGIN_SRC emacs-lisp
     ...
     (if gnu-ls-installed
     (setq insert-directory-program "/usr/local/Cellar/coreutils/8.27/bin/gls"))
