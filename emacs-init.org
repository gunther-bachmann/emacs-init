                    ...
                    (when-let* ((prop-fn fn))
                      (->>
                       (if-let* ((project (when (string-prefix-p (format "/home/%s/repo/" user-login-name) default-directory)
                                            (project-projectile default-directory)))
...
- accenting windows
  ...
  #+begin_src emacs-lisp
    ...
    (defun accented-window--add-window-accent (&optional window)
      (let ((win (or window (selected-window))))
        (unless (equal win accented-window--currently-accented-window)
          (unless (eql 1 (car (window-margins win)))
    ...
    (defun accented-window--window-change-hook (&rest args)
      (interactive)
      (let ((visible-frames (if (fboundp 'exwm-workspace--active-p) 
    ...
    (define-minor-mode accented-window-mode
      "minor mode to activate to accent windows on change"
      :lighter " A"
      (if accented-window-mode
          (progn
        ...
        (progn
          (remove-hook 'window-configuration-change-hook #'accented-window--window-change-hook)
          (remove-hook 'window-state-change-hook #'accented-window--window-change-hook))))

    (accented-window-mode 1)
  #+end_src
