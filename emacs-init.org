...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package helm
    :ensure helm
    :after ( async popup rainbow-delimiters)
    :bind (("M-x" . helm-M-x)
...
** dired sidebar
   ...
   (use-package dired-sidebar
     ...
     :after (dired dired-subtree all-the-icons)
     :init
     (progn
      ...
      (when gb/dual-monitor-setup
        (progn
          ...
          (add-hook 'exwm-randr-screen-change-hook
                    (lambda ()
                      (shell-command "xrandr --output DP-2 --left-of DP-0 --auto")
                      (let* ((xrandrmode (s-trim (shell-command-to-string "xrandr -q | grep DP-0 -A 1 | tail -n 1 | awk '{ print $1; }'"))))
                        (shell-command (s-concat "xrandr --output DP-0 --mode " xrandrmode)))))
          (exwm-randr-enable)))
     ...
     (when (and (file-exists-p "~/.emacs.d/additionals/font-lock+.el")
       ...
       (progn
         ...
         (when (file-exists-p "~/.emacs.d/additionals/sidebar.el/sidebar.el")
           ...
           (use-package sidebar
             :after ( icons-in-terminal projectile s dash dash-functional ov frame-local)
