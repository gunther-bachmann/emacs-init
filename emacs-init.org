    ...
    (when gb/use-exwm
      ...
      (req-package exwm-config
        ...
        (setq gb/simulation-keys '(
                ...
                ([?\C-y] . ?\C-v)
                ;; marking
                ([?\C-x ?h] . ?\C-a) ;; will work in emacs 27 first time
                ;; other
                ([?\C-g] . escape)
                ;; search
                ([?\C-s] . ?\C-f)))
                    ...
                    (when (and exwm-class-name
                      ...
                      (progn
                        ...
                        (let ((sim-keys (append gb/simulation-keys '(
                           ([33554451] . C-k) ;; search with search engine (C-S-s)
                           ([134217747] . f6) ;; jump into adresse bar (C-M-s)
                           ))))
     ...
     (when (and (file-exists-p "~/.emacs.d/additionals/font-lock+.el")
       ...
       (progn
         ...
         (when (file-exists-p "~/.emacs.d/additionals/sidebar.el/sidebar.el")
           (req-package sidebar
             :require icons-in-terminal projectile s dash dash-functional ov
