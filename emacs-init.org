        ...
        (let ((available-width
               (- (- (window-total-width) 1)
                  (+ (length (format-mode-line left))
                     (length (replace-regexp-in-string ":[a-zA-Z]+:" "LL" (format-mode-line right)))))))          
          (append left
                  (when (> available-width 0) (list (format (format "%%%ds" available-width) "")))
                  right)))

      (defun get-project-relative-buffer-filename ()
        (when-let* ((fn (buffer-file-name))
                    (prop-fn (propertize-base-name fn)))
          (if-let* ((project (project-current))
                    (project-root (replace-regexp-in-string "/$" "" (expand-file-name (cdr project)))))
              (replace-regexp-in-string (regexp-quote project-root) (concat (propertize "-" 'face '(:foreground "orange"))) prop-fn)
            (replace-regexp-in-string (regexp-quote (expand-file-name "~")) (propertize "~" 'face '(:foreground "green")) prop-fn))))
            ...
            (list '((:eval
                  ...
                  '(:eval 
                    (propertize (all-the-icons-icon-for-mode major-mode
                                                             ...
                                                             :v-adjust -0.03
                                                             (unless (gb/modeline-line-selected-window-active-p) :face)
                                                             (unless (gb/modeline-line-selected-window-active-p) 'mode-line-inactive))))
                  ;; Show the current major mode name
                  " [" 'mode-name "] "

                  ;; Show the file name with sanitized
                  '(:eval (or (if (gb/modeline-line-selected-window-active-p)
                                 (propertize-base-name (get-project-relative-buffer-filename))
                               (substring-no-properties (propertize-base-name (get-project-relative-buffer-filename))))
                             mode-line-buffer-identification))))

      (setq-default mode-line-format
                    (list '(:eval (simple-mode-line-render left-global-mode-string
                                                           (append global-mode-string (list mode-line-process))))))
