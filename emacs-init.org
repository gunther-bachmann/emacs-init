...
* EVAL pin xref to lower (installed) version 1.2.2
  ...
  #+begin_src emacs-lisp
    (use-package xref
      ...
      :pin manual)
  #+end_src
* coterm - support for tui apps in shell (disabled)
...
* EVAL undo-hl
  #+begin_src emacs-lisp
    (when (file-exists-p "~/.emacs.d/additionals/undo-hl.el")
      (use-package undo-hl
        :load-path "~/.emacs.d/additionals"
...
* EVAL dogears
  ...
  #+end_src
* EVAL jit lock status in modeline
  #+begin_src emacs-lisp :tangle no
...
* sticky-shell (shell header)
  #+begin_src emacs-lisp
    (use-package sticky-shell
      :load-path "~/repo/emacs/sticky-shell"
