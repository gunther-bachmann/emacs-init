...
** todo define something like layers
   ...
   file
** todo define keybindings depending on keyboard layout => e.g. key-chords won't make sense in us-keyboard layout 
...
** todo accepting invitations go into agenda, syncing google calendar goes into gcal
...
* initial accounts setup
  #+BEGIN_SRC emacs-lisp
  (setq agenda-accounts-exists (file-exists-p "~/.emacs.d/accounts/agenda-accounts.el"))
  (setq calendar-accounts-exists (file-exists-p "~/.emacs.d/accounts/calendar-accounts.el"))
  (setq mail-accounts-exists (file-exists-p "~/.emacs.d/accounts/mail-accounts.el"))
  #+END_SRC
...
#+END_SRC
** add el as package addon (disabled)
   el allows the installation of non repository packages by using recipes
   these recipes describe how to download (and compile) the given package
   - el-get-package-list
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  (el-get 'sync)
#+END_SRC
** bootstrap and initialize req-package
*** to see the log of req-package, use "M-x req-package--log-open-log"
#+BEGIN_SRC emacs-lisp
  (require 'req-package)
  ...
  #+BEGIN_SRC emacs-lisp
    (if (is-ms-windows-p)
        (progn
          ...
          ;; proxy settings TODO load from separate file
          (setq url-proxy-services
                '(("no_proxy" . "^\\(localhost\\|10.*\\)")
                  ("http" . "proxy.system.local:80")
                  ("https" . "proxy.system.local:80")))

          (setq graphviz-installation-path "C:/dev/tools/Graphviz 2.28/")

          (check-windows-program-status graphviz-installed (concatenate graphviz-installation-path "bin/dot.exe") "Graphviz" "org babel will not be able to prodcess dot files.")
          ...
          (check-windows-program-status everything-installed (concatenate everthing-installation-path "es.exe") "Everything" "Helm locate will not work without 'Everthing' installed")
          ...
          (check-windows-program-status hub-installed (concatenate magithub-installation-path "hub.exe") "Hub" "cannot use magithub within magit.")

          (if hub-installed
            (magithub-hub-executable "c:/dev/tools/hub/bin/hub.exe"))
         ...
         (check-windows-program-status aspell-installed (concatenate aspell-installation-path "bin/aspell.exe") "Aspell" "no spell checking will be possible")

         (if aspell-installed
             (progn
               (add-to-list 'exec-path (concatenate aspell-installation-path "bin/"))
...
** add theme
   #+BEGIN_SRC emacs-lisp
     ;; TODO change to use varaible user-init-file, chop off init.el at the end and use it as path
     (if (file-exists-p "~/.emacs.d/themes")
         (progn
           (add-to-list 'custom-theme-load-path "~/.emacs.d/themes")
           (add-to-list 'load-path "~/.emacs.d/themes")
           (if (file-exists-p "~/.emacs.d/themes/sanityinc-tomorrow-night-theme.el")
               (load-theme 'sanityinc-tomorrow-night t)
             (message "WARNING: theme folder '~/.emacs.d/themes' does not contain 'sanityinc-tomorrow-night")))
       (message "WARNING: theme folder '~/.emacs.d/themes' not found. Themes will not be loaded"))
   #+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (req-package transpose-frame
    :bind (
    ("C-s-+" . flip-frame) ;; exchange top with bottom windows
...
** sytem stats (for minibuffer)
   #+BEGIN_SRC emacs-lisp
     ...
     while true; do
         ...
         s=`ps -e -o %%cpu | awk '{x+=$1} END {print x/8}' | sed 's/,.*//'`
         echo \"cpu:$s\"
...
*** spaceline (mode line of spacemacs)
    #+BEGIN_SRC emacs-lisp
           (req-package spaceline
             ...
             (spaceline-define-segment gb/buffer-file-name
               ""
               ;; TODO: limit path length to a certain number of characters
               ;; TODO: allow replacing certain paths with some "propertized" string of shorthands (e.g. :doc: for ~/Documents/)
               ;; TODO: scratch buffer does not show any modeline
             ...
             (apply 'spaceline--theme
               '(gb/buffer-status buffer-size gb/buffer-file-name buffer-id remote-host)
               '(paradox-menu persp-name)
               ;; additional-segments
               )
      ;;       (spaceline-install
      ;;        'main
      ;;        '((gb/buffer-status)
      ;;          (gb/buffer-file-name remote-host buffer-id)
      ;;          (process :when active))
    ...
    #+END_SRC
*** rich minority mode (hide minor modes from smart mode line) (disabled)
    #+BEGIN_SRC emacs-lisp :tangle no
...
** toggle commands (C-x t ...)
   #+BEGIN_SRC emacs-lisp
     ...
     (define-key gb/toggle-map "u" #'focus-mode)
   ...
   (req-package flyspell
     ...
     disable spell checking if `#+PROPERTY: spellcheck no' is set"
       (let* ((fileLang (plist-get (org-export--get-inbuffer-options) :language))
              (spellCheck (gb/org-global-prop-value "spellcheck")))
         (if (string= spellCheck "no")
             (progn (flyspell-mode 0))
           (progn (if (string= fileLang "de")
  ...
  (req-package company
    ...
    (progn
      ...
      (setq company-dabbrev-code-modes
            (quote
             (prog-mode batch-file-mode css-mode erlang-mode haskell-mode jde-mode lua-mode python-mode clojure-mode scala-mode)))
...
#+BEGIN_SRC emacs-lisp
  ...
  (req-package ace-window
    ...
    :init
    (when (package-installed-p 'key-chord)
      (key-chord-define-global "o0"   'ace-window))) ;; select open window with one key
...
#+BEGIN_SRC emacs-lisp :tangle no
  (req-package ace-isearch
    ...
    (progn
      ...
      ;; C-u before M-p swaps active with selected window!
      ;; starts working only of more than one buffer is open (exluding speedbar!)
...
** shell here
   ...
   #+END_SRC
** shell ansi colors configuration
   #+BEGIN_SRC emacs-lisp
...
;;   '(initial-frame-alist (quote ((fullscreen . maximized))))
   ...
   '(ps-font-size (quote (7 . 6.5)))
   '(ps-paper-type (quote a4))
   '(recentf-max-saved-items 80)
   '(region ((t (:background "gray45" :inverse-video nil))))
   '(rng-schema-locating-files
     (quote
      ("~/.emacs.d/ant-mode/ant-schemas.xml" "schemas.xml" "~/devel/xspec-read-only/editors/emacs/schemas.xml" "/Applications/Emacs.app/Contents/Resources/etc/schema/schemas.xml")))
...
#+BEGIN_SRC emacs-lisp
  (req-package yasnippet
    ...
    (setq yas-snippet-dirs
          '("~/.emacs.d/snippets")) ;; my snippets library
...
*** org
  #+BEGIN_SRC emacs-lisp
    (req-package org
      ...
      :config
      (require 'ob-clojure)
      (defun gb/org-edit-special ()
        "make sure to remove read only mode if going into edit special"
        (interactive)
        (read-only-mode -1)
        (org-edit-special))
      (bind-key "C-c '" 'gb/org-edit-special org-mode-map)
      ;; make sure that blackground of src block is black
...
** flymd
   #+BEGIN_SRC emacs-lisp
   (req-package flymd) ;; allow live preview of markdown
   #+END_SRC
...
** wolfram (currently disabled)
   ...
   (req-package wolfram)
   #+END_SRC
** major mode icons (disabled, does not work with powerline)
   #+BEGIN_SRC emacs-lisp :tangle no
