         ...
         (cond
          ...
          (t (progn
               (--each '((link-face ((t (:foreground "#81a2be" :underline (:style line :color "#41526e")))))
                 ...
                 (add-to-list 'gb/list-of-faces-to-set it)))))))
   #+END_SRC
          ...
          (progn
            ...
            (defun gb/pdf-view-scroll-up-or-next-page (orig-func &rest args)
              ...
              (let ((vscroll-before (window-vscroll)))
                ...
                (when (= vscroll-before (window-vscroll)) ;; if scroll position before is exactly the same as after, jump to next page
        ...
        (cond
         ...
         (t (progn
              (--each '( (header-line ((t (:inherit org-table-face :foreground "#8abeb7")))))
                (add-to-list 'gb/list-of-faces-to-set it))))) ;; set (non scrolling) header to inhiert font from org mode tables (not mode line, which is to small on multi monitor w/ current config)
...
* TODO EVAL git-messenger - popup last commit information for line (uses popup should use posframe)
  ...
  #+begin_src emacs-lisp
    (use-package git-messenger
      :ensure t
      :config
      (bind-key "C-c v p" 'gb/git-messenger:popup-message)

      (bind-key "m" 'git-messenger:copy-message git-messenger-map)

      ;; Use magit-show-commit for showing status/diff commands
      (setq git-messenger:use-magit-popup nil)
  ...
  #+end_src
* EVAL gitconfig-mode - mode to edit git config files
