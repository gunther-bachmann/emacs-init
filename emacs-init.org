...
- accenting windows
  ...
  #+begin_src emacs-lisp
    ...
    (defvar animated-face-timer nil)
    (defvar gb/animate-face-background-color-step 0)
    (defvar gb/gradient nil)
    ...
    (defun gb/add-window-accent (&optional window)
      (let ((win (or window (selected-window))))
        (unless (equal win gb/currently-accented-window)
          ...
          (gb/background-for-window-accent)
          (setq gb/currently-accented-window win)
          (setq gb/currently-accented-buffer (window-buffer win))
          (when animated-face-timer
            (cancel-timer animated-face-timer))
          (setq animated-face-timer nil)
          (setq gb/animate-face-background-color-step 0)
          (setq animated-face-timer (run-at-time 0.5 0.1 #'gb/animate-face-background-color)))))
    ...
    (defconst gb/accented-background-gradient-orange
      '("#cd8500" "#9c5c02"  "#824f04" "#684106" "#4c3d08"  "#34250a" "#1a170d" "#161414")
      "list of values applied each after .1 second")

    (defconst gb/accented-background-gradient-purple
      '("#402040" "#341e34"  "#321b32" "#2b192b" "#261726"  "#201620" "#1b151b" "#161414")
      "list of values applied each after .1 second")

    (defun gb/animate-face-background-color ()
      (when gb/gradient
        (set-face-background 
         'fringe 
         (nth gb/animate-face-background-color-step gb/gradient))
        (setq gb/animate-face-background-color-step (1+ gb/animate-face-background-color-step))
        (when (> gb/animate-face-background-color-step 7)
          (when animated-face-timer
            (cancel-timer animated-face-timer))
          (setq gb/animate-face-background-color-step 0)
          (setq animated-face-timer nil))))
      ...
      (with-selected-window (or window (selected-window))
        (cond ((or (minibufferp)
              ...
              ((or exwm-class-name
               ...
               (setq gb/gradient gb/accented-background-gradient-orange))
              (t
               (set-face-background 'fringe "#402040")
               (setq gb/gradient gb/accented-background-gradient-purple)))))
    ...
    (defun gb/selected-window-accent (&rest args)
      ...
      (if (or (minibufferp))
          (progn
            ...
            (gb/background-for-window-accent))
        (progn  
          (unless (or (equal " *my-posframe-buffer*" (buffer-name))
                     (minibufferp)
