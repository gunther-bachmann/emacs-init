...
* startup profile configuration
  #+begin_src emacs-lisp
    ...
    (setq gb/use-frame-completion 'mini-frame) ;; 'ivy-posframe, 'mini-frame, nil
  #+end_src
* startup
#+begin_src emacs-lisp
...
** other
   #+BEGIN_SRC emacs-lisp
     ...
     (add-hook 'text-mode-hook #'mouse-avoidance-mode)
     (add-hook 'comint-mode-hook #'mouse-avoidance-mode)
     (add-hook 'lisp-interaction-mode-hook #'mouse-avoidance-mode)
     (add-hook 'special-mode-hook #'mouse-avoidance-mode)
   #+END_SRC
** window/frame
   #+BEGIN_SRC emacs-lisp
...
** modeline
   #+begin_src emacs-lisp
     (cond
      ((eq gb/theme-mode 'light) nil) ;; TODO complete
      (t (progn
           (--each '((mode-line ((t (:background "#251422" :foreground "#A5A5A5" :overline "#504050" :underline "#504050" :weight normal))))
...
*** custom modeline
    #+begin_src emacs-lisp
      ...
      (defconst ml-sudo-prefix (concat (propertize "sudo" 'face '(:foreground "red")) ":"))
      (defconst ml-n-store-prefix (concat (propertize "[n-store]" 'face '(:foreground "#A01010")) ":"))
      (defconst ml-gnu-store-prefix (concat  (propertize "[store]" 'face '(:foreground "#A01010")) ":"))
      (defconst ml-src-prefix (concat ":" (propertize "[src]" 'face '(:foreground "#A01010")) ":"))
      (defconst ml-test-prefix (concat ":" (propertize "[tst]" 'face '(:foreground "#10A010")) ":"))
      (defconst ml-home-prefix (propertize "~" 'face '(:foreground "green")))
      (defconst ml-project-prefix (propertize "-" 'face '(:foreground "orange")))
      (defconst ml-timestamp-prefix (propertize "⏱ " 'face '(:foreground "blue")))

      (require 'color)
      (defconst mode-line-file-name-highlight-color "orange")
      (defconst mode-line-file-name-disabled-color (color-darken-name mode-line-file-name-highlight-color 25))
                  ...
                  (when-let* ((prop-fn fn))
                    (->>
                     (if-let* ((project (project-current nil default-directory))
                         ...
                         (gb/string-replace-prefix project-root ml-project-prefix prop-fn)
                       (gb/string-replace-prefix (expand-file-name "~") ml-home-prefix prop-fn))
                     (gb/string-replace-prefix "/sudo:root@pe-nixos:" ml-sudo-prefix)
                     (replace-if-starting-with "/nix/store/" "/nix/store/[a-z0-9]+-" ml-n-store-prefix)
                     (replace-if-starting-with "/gnu/store/" "/gnu/store/[a-z0-9]+-" ml-gnu-store-prefix)
                     (replace-regexp-in-string "/src/main/\\(java\\|scala\\|kotlin\\)/" ml-src-prefix)
                     (replace-regexp-in-string "/src/test/\\(java\\|scala\\|kotlin\\)/" ml-test-prefix)
                     (replace-regexp-in-string "[12][019][0-9][0-9][01][0-9][0-3][0-9][0-2][0-9][0-5][0-9][0-5][0-9]-" ml-timestamp-prefix))))
                ...
                (cond
                 ...
                 ((not (buffer-modified-p))
                  ...
                  '(:eval (let* ((id        (or (get-project-relative-buffer-filename)
                                 ...
                                 (id-is-str (stringp id)))
                              (if (gb/modeline-line-selected-window-active-p)
                                  id
                                (propertize  (if id-is-str
                                                 (substring-no-properties id)
                                               (substring-no-properties (format-mode-line id)))
                                             'face (if (gb/modeline-line-selected-window-active-p)
                                                       `(:foreground ,mode-line-file-name-highlight-color) ;; "orange"
                                                     (if id-is-str
                                                         `(:foreground ,mode-line-file-name-disabled-color) ;; "DarkGoldenrod4"
                                                       `(:foreground ,mode-line-buffer-name-disabled-color))))))) ;; "#72547b" DarkOrchid3 #48226B b284bb
   ...
   #+BEGIN_SRC emacs-lisp
     (when (eq gb/completion-framework 'ivy)
       ...
       (use-package all-the-icons-ivy
         :ensure t)
       (defun gb/ivy-candidates-offset (cand) (make-string (if mini-frame-mode 0 (max 0 (- (frame-width) (ivy-rich-minibuffer-width 0.77)))) ? ))
       (defun gb/ivy-candidates-offset-20 (cand) (make-string (if mini-frame-mode 0 20) ? ))
       (defun gb/prefixed-20-all-the-icons-ivy-rich-buffer-icon (cand)
         (concat (gb/ivy-candidates-offset-20 cand) (all-the-icons-ivy-rich-buffer-icon cand)))
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (use-package magit
        ...
        :custom ((magit-fetch-arguments (quote ("--prune")))
                 ...
                 (with-editor-emacsclient-executable (s-trim (shell-command-to-string "command -v emacsclient")))
        ...
        (when (string= major-mode "org-mode")
          (progn
            (let ((has-source-regions (gb/buffer-contains-string "^ *#\\+BEGIN_SRC"))
                  ...
                  (branch-name (or (ignore-errors (string-trim (shell-command-to-string "git branch --show-current"))) "any"))
                  (tangle-branch (or (ignore-errors (string-trim (shell-command-to-string "cat ~/.tangle-branch-name.txt"))) ""))
...
*** insertion of web images
    ...
    #+BEGIN_SRC emacs-lisp :tangle no
      (use-package org-easy-img-insert
        ...
        :config (progn))
    #+END_SRC
*** *DISABLED* org reveal (presentation)
    #+BEGIN_SRC emacs-lisp :tangle no
       ...
       #+begin_src emacs-lisp
         ...
         (defun gb/compositor-enable ()
           ...
           (unless (= 0 (shell-command "pgrep -x compton"))
             (shell-command "nvidia-settings -a 'AllowFlipping=0'") ;; prevent emacs keyboard delay
    ...
    - cpu frequency
      #+begin_src emacs-lisp
        ...
        (defun gb/cpu-base-frequency ()
          "in Hz"
          (string-to-number (shell-command-to-string "cat /sys/devices/system/cpu/cpu0/cpufreq/base_frequency")))

        (defun gb/cpu-min-frequency ()
          "in Hz"
          (string-to-number (shell-command-to-string "cat /sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_min_freq")))

        (defun gb/cpu-max-frequency ()
          "in Hz"
          (string-to-number (shell-command-to-string "cat /sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq")))

        (defun gb/cpu-core-count ()
          "get the number of cpus in the system"
          (string-to-number (shell-command-to-string "ls /sys/devices/system/cpu/cpu* -d1 | grep -e \"/cpu[0-9]\" | wc -l")))
    ...
    #+begin_src emacs-lisp
      (use-package org-tree-slide
        ...
        (when (fboundp 'valign-mode)
          (advice-add 'org-tree-slide-after-narrow-hook :after #'gb/align-all-tables))
      ...
      (if (not (executable-find "sqlite3"))
        ...
        (progn
          ...
          (use-package org-roam
            ...
            :after (org emacsql-sqlite notdeft)
  ...
  #+begin_src emacs-lisp
    (when (eq gb/use-frame-completion 'ivy-posframe)
      (use-package ivy-posframe
        ...
        (setq ivy-posframe-min-width 30)
...
* github notifications
  #+begin_src emacs-lisp
    ...
    (defun gb/refresh-polybar-github ()
      (shell-command-to-string "polybar-msg action github hook 0"))
...
* EVAL ca65-mode
  ...
  #+end_src
* EVAL inspector
  - inspector - Tool for inspection of Emacs Lisp objects. 
  #+begin_src emacs-lisp
...
* EVAL dogears
  ...
  #+end_src
* EVAL vale - spell checker for org mode
  - works in org mode!
      ...
      (defun gb/select-mounted-folder ()
        ...
        (let ((uname (user-login-name)))
          (when (file-exists-p (format "/run/media/%s" uname))
            (shell-command-to-string (format "sudo chown %s:users /run/media/%s/" uname uname)))
  ...
  - en/disable touchpad depending on currently visited buffer
    #+begin_src emacs-lisp
      (if (not (executable-find "set-touchpad"))
        ...
        (setq touchpad-active (string-equal "on" (string-trim (shell-command-to-string "get-touchpad"))))
      ...
      disable touchpad for all other buffers"
          (let* ((should-activate-touchpad (exwm--buffer->id (current-buffer))))
            (when (and (not (bound-and-true-p gb/running-exwm-monitor-setup))
                     (xor should-activate-touchpad touchpad-active))
              (ignore-errors
                (with-ignored-messages
                 (shell-command-to-string (format "set-touchpad %s" (if should-activate-touchpad "on" "off")))
