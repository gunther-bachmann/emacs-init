...
** printer
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defun gb/query-default-printer ()
       ...
       (interactive)
       ;; (ivy-read "printer:" (gb/get-known-printers) :preselect (gb/get-standard-printer) :require-match t)
       (let ((printer-name (helm-comp-read "printer:" (gb/get-known-printers) :default (gb/get-standard-printer) :must-match t)))
...
** company
   #+BEGIN_SRC emacs-lisp
     ...
     (use-package company-box
       ...
       (defun window-tab-line-height () 0) ;; currently not available (will be there on emacs 27)
       :config
       (progn
         ...
         (defun company-box--update-scrollbar-buffer (height-blank height-scrollbar percent buffer)
           (with-current-buffer buffer
             ...
             (unless (zerop height-blank)
               (insert (propertize " " 'display `(space :align-to left-fringe :height ,height-blank))
                       (propertize "\n" 'face '(:height 1))))
             ...
             (insert (propertize " " 'face (list :background (face-background 'company-box-scrollbar nil t))
                                 'display `(space :align-to left-fringe :height ,height-scrollbar)))
             (current-buffer)))
...
** kotlin
   #+BEGIN_SRC emacs-lisp
     ...
     (use-package gb-kotlin
       ...
       :after (kotlin-mode magit)
       :config
       (bind-key "C-c M-w"   #'gb/kotlin-kill-full-qualified-class-name-at-point kotlin-mode-map)
       (bind-key "C-c / i i" #'gb/kotlin-import-at-point kotlin-mode-map) ;; insert
       (bind-key "C-c / i t" #'gb/kotlin-import-test-insert kotlin-mode-map) ;; test
       (bind-key "C-c / i s" #'gb/kotlin-import-stdlib-insert kotlin-mode-map) ;; stdlib
       (bind-key "C-c / i r" #'gb/kotlin-rewrite-import-region kotlin-mode-map) ;; rewrite
       (bind-key "C-c / i l" #'gb/kotlin-import-search-and-insert kotlin-mode-map)) ;; lookup
   #+END_SRC
** haskell
   #+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (use-package helm-backup
    :ensure t
    :init (add-hook 'after-save-hook #'helm-backup-versioning)) ;; after-save-hook
#+END_SRC
* latex (auctex)
** auctex
   #+BEGIN_SRC emacs-lisp
    ...
    - audio functions
       #+begin_src emacs-lisp
         ...
         (defun gb/audio-lower-volume ()
           "lower volume by 5%"
           (interactive)
           (shell-command (format "pactl set-sink-volume %s -5%%" (gb/audio-output-selected)))
           (message (format "volume down, now %s     (%s)" (gb/get-volume-lr-escaped) (gb/audio-output-selected))))

         (defun gb/get-volume-lr-escaped ()
           "get current volume as string"
           (s-replace "%" "%%" (s-trim (shell-command-to-string (format "pactl list sinks | grep -A 9 \"Name: %s\" | grep -e \"^[[:space:]]*Volume:\" | awk '{ print $5 \" / \" $12 }'" (gb/audio-output-selected))))))

         (defun gb/audio-raise-volume ()
           "raise volume by 5%"
           (interactive)
           (shell-command (format "pactl set-sink-volume %s +5%%" (gb/audio-output-selected)))
           (message (format "volume up, now %s     (%s)" (gb/get-volume-lr-escaped)(gb/audio-output-selected))))
         ...
         (defun gb/audio-output-select ()
           "select audio output "
           (interactive)
           ;; (ivy-read "audio-output:" (gb/audio-outputs-available) :require-match t)
           (let ((sink-name (helm-comp-read "audio-output:" (gb/audio-outputs-available) :must-match t)))
         ...
         (defun gb/audio-input-select ()
           "select audio input"
           (interactive)
           (let ((source-name (helm-comp-read "audio-input:" (gb/audio-input-available) :must-match t)))
      ...
      (when gb/use-exwm
        (use-package exwm-config
          ...
          (defun gb/exwm--get-geometry-str (monitor)
            ""
            (car (s-split "+" (s-trim (shell-command-to-string (format "xrandr | grep '%s connected' | cut -f3 -d' '" monitor))))))
...
* networking en/disable
  #+begin_src emacs-lisp
    ...
    (defun gb/net-vpn-switch-on ()
      "switch vpn on"
      (interactive)
      (let ((vpn-id (helm-comp-read "vpn:" (gb/get-vpns-available) :must-match t)))
...
* EVAL org roam
  ...
  #+end_src
* EVAL emacs-mini-frame (disabled)
  - now available as melpa package (but not patched yet)
  - mini-frame-do-commands was added by me
  - BUG: currently placing the popup window behind other xwindows
  #+begin_src emacs-lisp :tangle no
    (use-package mini-frame
      ...
      ;; (setq mini-frame-do-commands
      ;;       ;; '() ;; when using the empty list instead of the other, only the ones explicitly ignored will not use the mini frame
      ;;       '(exwm-workspace-switch
      ;;         gb/query-default-printer
      ;;         gb/net-vpn-switch-on
      ;;         gb/net-vpn-switch-off
      ;;         eval-expression
...
* EVAL maxima
  #+begin_src emacs-lisp
    (setq maxima-elisp-loadpath (s-concat (s-trim (shell-command-to-string "dirname $(readlink -f $(command -v maxima))")) "/../share/emacs/site-lisp"))

    (use-package imaxima
      :load-path maxima-elisp-loadpath
      :commands (imaxima)
      :ensure-system-package maxima
      :config (setq imaxima-fnt-size "LARGE")
      )
    (use-package imath
      :load-path maxima-elisp-loadpath)
    (use-package maxima
  ...
  #+begin_src emacs-lisp
    (use-package smtpmail-async
      ...
      (defun gb/gnus-mail-send-switch-to-safe-sync ()
        "save to save sync mode of sending gnu mails"
        (interactive)
      ...
      (defun gb/gnus-mail-send-switch-to-unsafe-async ()
        "save to unsafe async mode - might fail, if additional certificate acknowledgment is necessary"
        (interactive)
