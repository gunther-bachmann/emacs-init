...
* define standard alias
  ...
  #+begin_src sh :eval no :tangle ~/.direnvrc
    export_alias a "# get info of all defined aliases" "for al in \$(ls -1 \${DIRENV_DIR:1}/.direnv/aliases); do echo -n \$al; echo -ne \"\\t\"; \$al --help; done;"
  #+end_src
* guix
  ...
  #+begin_src sh :eval no :tangle ~/.direnvrc
    ...
    use_guix() {
      eval "$(guix time-machine --commit=38ef37d4bb684849e1789d57e46a039e6d005579 -- shell "$@" --search-paths)"
    }
  #+end_src
* nix
